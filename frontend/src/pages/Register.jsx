...
export default function Register() {
  ...
  // Track window width for responsive grid logic
  useEffect(() => {
  ...
  const validateForm = () => {
    ...
    if (formData.role === 'pro') {
      ...
      if (!formData.skills) newErrors.skills = "Trade is required";
    }
    return newErrors;
  };
    ...
    <div style={styles.pageContainer}>
      ...
      <div style={styles.card} className="register-card">
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ ...styles.formGrid, gridTemplateColumns: getGridTemplate() }}>
...
const styles = {
  ...
  title: { margin: 0, fontSize: 'clamp(1.5rem, 5vw, 2rem)', fontWeight: '800', color: '#1d4ed8' },
  ...
  input: { padding: '12px', borderRadius: '12px', border: '1px solid #e0e7ff', fontSize: '0.95rem', fontWeight: '600', outline: 'none', width: '100%', boxSizing: 'border-box' },
  errorText: { color: '#dc2626', fontSize: '0.7rem', fontWeight: '700', marginTop: '4px' },
  toggleGroup: { display: 'flex', gap: '12px' },
  toggleBtn: { flex: 1, padding: '12px', borderRadius: '12px', cursor: 'pointer', fontWeight: '700', border: '1px solid #e0e7ff', transition: 'all 0.2s' },
  buttonContainer: { marginTop: '28px' },
  button: { width: '100%', padding: '14px', color: '#fff', border: 'none', borderRadius: '40px', fontSize: '1rem', fontWeight: '800', cursor: 'pointer', transition: 'background 0.2s' },
  footerText: { textAlign: 'center', marginTop: '20px', fontWeight: '600', fontSize: '0.9rem' },
  link: { color: '#1d4ed8', textDecoration: 'none', fontWeight: '800' },
  notification: { position: 'fixed', top: '20px', right: '20px', padding: '14px 18px', borderRadius: '16px', background: '#ffffff', boxShadow: '0 15px 30px rgba(0,0,0,0.1)', borderLeft: '4px solid', display: 'flex', alignItems: 'center', gap: '12px', zIndex: 9999 },
